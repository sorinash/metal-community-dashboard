...
2. The band with the highest degree (number of connections to other bands) within that community.
3. The degree of that band.
4. The average degree of bands in the cluster.
5. The number of end branches (ie, bands that only connect to one other band)
6. The assortativity of the community (ie, the tendency of high-degree bands to associate with other high-degree bands), ranging from -1 to 1.
7. The clustering coefficient of the community (how closely connected the bands are to one another)
8. The density of the community (what proportion of possible connections between bands actually exist; expect this to be low).
9. A choropleth map detailing the number of bands per country in the given community; hovering over each country will give the number of bands in that country. Note that the scale changes based on the number of bands in the community; I'll include a color bar in future releases. 
